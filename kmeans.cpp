...
vector<size_t> KMeans(const Points& data, size_t K) {
    ...
    while (!converged) {
        ...
        vector<vector<size_t> > clusters_sizes_threads(max_threads, vector<size_t>(K));
        centroids.assign(K, Point(dimensions));
        vector<vector<Point> > centroids_thread(max_threads, vector<Point>(K, Point(dimensions)));
        size_t d;
        int thread_id;
        #pragma omp parallel for private(i, d, thread_id)
        for(i = 0; i < data_size; i++)
        {
            thread_id = omp_get_thread_num();
            for(d = 0; d < dimensions; d++)
                centroids_thread[thread_id][clusters[i]][d] += data[i][d];
            ++clusters_sizes_threads[thread_id][clusters[i]];
        }
        for(size_t i = 0; i < max_threads; i++)
        {
            ...
            {
                for(size_t d = 0; d < dimensions; d++)
                    centroids[j][d] += centroids_thread[i][j][d];
                clusters_sizes[j] += clusters_sizes_threads[i][j];
            }
        }
        for (size_t i = 0; i < K; ++i) {
...
int main(int argc , char** argv) {
    ...
    fprintf(stderr, "Threads: %2d, time: %.10lf\n", omp_get_max_threads(), end - start);
    return 0;
}
